...
pub struct Discovery<'a> {
	...
	//这个字段是什么意思？ 指的是这个结点已经把 pingRequest发出去了
...
impl<'a> Discovery<'a> {
	...
	pub fn add_node(&mut self, e: NodeEntry) {
		...
		let id_hash = keccak(e.id);//sha256
	...
	fn on_ping(&mut self, rlp: &Rlp, node_id: &NodeId, from: &SocketAddr, echo_hash: &[u8]) -> Result<Option<TableUpdates>, Error> {
		...
		if !entry.endpoint.is_valid() {
			...
			//节点在接受到ping 请求后，在进行一系列的验证后，也进行节点发现过程
	...
	fn on_pong(&mut self, rlp: &Rlp, node_id: &NodeId, from: &SocketAddr) -> Result<Option<TableUpdates>, Error> {
		...
		//从正在ping的队列里面把该节点对应的entry取出来，做信息的校验
...
fn assemble_packet(packet_id: u8, bytes: &[u8], secret: &Secret) -> Result<Bytes, Error> {
	//bytes.len 的长度观察到的值为31 或者49
	let mut packet = Bytes::with_capacity(bytes.len() + 32 + 65 + 1);//为什么要初始化这么大容量的字节数组？ 现在理解，这样操作 是为了减少变量的初始化，使用一个bytes 就能满足在后续环节的计算使用
	packet.resize(32 + 65, 0); // Filled in below
	packet.push(packet_id);
	packet.extend_from_slice(bytes);//将bytes的值从packet 32+65的位置开始添加

	let hash = keccak(&packet[(32 + 65)..]);//计算bytes的hash
