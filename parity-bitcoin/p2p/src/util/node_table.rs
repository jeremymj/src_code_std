...
impl<T> NodeTable<T> where T: Time {
	...
	pub fn insert_many(&mut self, addresses: Vec<AddressEntry>) {
		...
		for addr in iter {
			...
			//通过地址来索引，查看当前地址是否在实例中
			match self.by_addr.entry(node.addr) {
				Entry::Occupied(mut entry) => {
					let old = entry.get_mut();
					// we've already seen this node，若这个地址已经存在，则比较它的时间戳，若是更新些，则更新该地址的时间戳
				...
				Entry::Vacant(entry)=> {
					// it's first time we see this node 假如是第一次看到该地址，则将地址insert 本地
					self.by_score.insert(node.clone().into());
					self.by_time.insert(node.clone().into());
					entry.insert(node);
				}
	...
	/// Marks address as recently used. 更新最近使用的节点时间
	pub fn note_used(&mut self, addr: &SocketAddr) {
		//先将节点从table里面删除，再将node insert into BTree结构缓存
	...
	pub fn save<W>(&self, write: W) -> Result<(), io::Error> where W: io::Write {
		let mut writer = csv::WriterBuilder::new()
			.delimiter(b' ')
			.from_writer(write);
			//将node节点中按照score的方式排名，取1000个节点，存储在本地文件中
