	...
	where Cr: Fn(String) + 'static + Send
{
	...
	println!("execute_light_impl user defaults path {:?}",user_defaults_path);
	// load user defaults
...
fn execute_impl<Cr, Rr>(cmd: RunCmd, logger: Arc<RotatingLogger>, on_client_rq: Cr,
	...
	where Cr: Fn(String) + 'static + Send,
		Rr: Fn() + 'static + Send
{
	//println!("execute_impl cmd detail is:{:?}",cmd);
	// load spec
	println!("&cmd.dirs.cache:{}",&cmd.dirs.cache);
	...
	// user defaults path
	let user_defaults_path = db_dirs.user_defaults_path();
	println!("execute_impl user default path is:{:?}",user_defaults_path);
	// load user defaults
	...
	// set up bootnodes
	let mut net_conf = cmd.net_conf;
	println!("boot nodes detail:{:?}",net_conf);
	...
	// set network path.
	net_conf.net_config_path = Some(db_dirs.network_path().to_string_lossy().into_owned());
	println!("net conf path:{:?}",net_conf.net_config_path);
	...
	// register it as an IO service to update periodically. 存储
