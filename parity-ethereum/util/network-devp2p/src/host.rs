...
pub struct Host {
	...
	sessions: Arc<RwLock<Slab<SharedSession>>>,
	discovery: Mutex<Option<Discovery<'static>>>,//可以这样创建？
...
impl Host {
	...
	pub fn new(mut config: NetworkConfiguration, filter: Option<Arc<ConnectionFilter>>) -> Result<Host, Error> {
		...
		println!("boot node size is:{:?},reserved_nodes size is:{:?}",boot_nodes.len(),reserved_nodes.len());
		for n in boot_nodes {
			host.add_node(&n);
		}
	...
	fn init_public_interface(&self, io: &IoContext<NetworkIoMessage>) -> Result<(), Error> {
		...
		println!("public_endpoint:{:?},local_endpoint:{:?} ",public_endpoint,local_endpoint);
		// Initialize discovery. 地址初始化
		let discovery = {
			let info = self.info.read();
		...
		if let Some(mut discovery) = discovery {
			...
			*self.udp_socket.lock() = Some(socket);
			//entries
			let entries = self.nodes.read().entries();

			//将本地
			discovery.add_node_list(entries);
